...
| Malformed/invalid TLS certificate upload | `CERTIFICATE_MANAGE` (admin-only); keystore parsed in-memory by a pure JS library, no shell-out; certificate expiry and private-key-matches-certificate checked before anything is written; atomic write (temp file + rename) so a failed upload never touches the certificate nginx is currently serving | integration test with a real generated keystore + bad-password/expired/mismatched-key fixtures |
...
Security-relevant events go to the `security` logger _and_ to `AuditLog`. Log
