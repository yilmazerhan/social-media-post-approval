...
import { getByDepartmentReport, toCsv } from "@/modules/reports";
...
/** `?from=&to=&departmentId=&priority=&format=json|csv` — API.md's `/reports/by-department`. */
export const GET = protectedHandler(
  ...
  async ({ request }) => {
    ...
    const rows = await getByDepartmentReport({
      ...
      return {
        raw: new NextResponse(csv, {
          headers: {
            ...
            "Content-Disposition": 'attachment; filename="by-department.csv"',
          },
        }),
      };
    }
