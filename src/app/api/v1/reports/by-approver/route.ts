...
import { getByApproverReport, toCsv } from "@/modules/reports";
...
/** `?from=&to=&departmentId=&priority=&format=json|csv` — API.md's `/reports/by-approver`. */
export const GET = protectedHandler(
  ...
  async ({ request }) => {
    ...
    const rows = await getByApproverReport({
      ...
      return {
        raw: new NextResponse(csv, {
          headers: {
            ...
            "Content-Disposition": 'attachment; filename="by-approver.csv"',
          },
        }),
      };
    }
