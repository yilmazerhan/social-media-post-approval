...
import { getSummaryReport, toCsv } from "@/modules/reports";
...
/** `?from=&to=&departmentId=&priority=&format=json|csv` — API.md's `/reports/summary`. */
export const GET = protectedHandler(
  ...
  async ({ request }) => {
    ...
    if (params.get("format") === "csv") {
      const csv = toCsv(
        [
          { metric: "Submitted", count: report.submitted },
          { metric: "Approved", count: report.approved },
          { metric: "Rejected", count: report.rejected },
          { metric: "Changes requested", count: report.changesRequested },
          { metric: "Overdue (currently)", count: report.overdue },
        ],
        [
