...
import { getThroughputReport, toCsv } from "@/modules/reports";
...
/** `?from=&to=&departmentId=&priority=&format=json|csv` — API.md's `/reports/throughput`. */
export const GET = protectedHandler(
  ...
  async ({ request }) => {
    ...
    const points = await getThroughputReport({
      ...
      return {
        raw: new NextResponse(csv, {
          headers: {
            ...
            "Content-Disposition": 'attachment; filename="throughput.csv"',
          },
        }),
      };
    }
